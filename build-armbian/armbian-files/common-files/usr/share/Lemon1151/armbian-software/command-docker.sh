...
software_118() {
    ...
    image_name="Lemon1151/openwrt-aarch64:latest"
    install_path="${docker_path}/${container_name}"
    ...
    install)
        ...
        # Instructions: https://hub.docker.com/r/Lemon1151/openwrt-aarch64
        docker run -d --name=${container_name} \
            -e PUID=${docker_puid} \
            -e PGID=${docker_pgid} \
            -e TZ=${docker_tz} \
        ...
        echo -e "${NOTE} The ${container_name} instructions [ https://hub.docker.com/r/Lemon1151/openwrt-aarch64 ]"
        echo -e "${SUCCESS} ${container_name} installed successfully."
        exit 0
        ;;
    update) docker_update ;;
