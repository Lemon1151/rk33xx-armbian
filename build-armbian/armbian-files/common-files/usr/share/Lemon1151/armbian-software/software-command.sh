...
check_release() {
    if [[ -f "${Lemon1151_release_file}" ]]; then
        source "${Lemon1151_release_file}" 2>/dev/null
        VERSION_CODEID="${VERSION_CODEID}"
        VERSION_CODENAME="${VERSION_CODENAME}"
    else
        error_msg "${Lemon1151_release_file} file is missing!"
    fi

    [[ -n "${VERSION_CODEID}" && -n "${VERSION_CODENAME}" ]] || error_msg "${Lemon1151_release_file} value is missing!"
}
